package endpoints

import (
	"fmt"
	//"github.com/Jeffail/gabs"
	"encoding/json"
	"github.com/jetlwx/k8s_admin/common"
	"github.com/jetlwx/k8s_admin/models"
	"github.com/jetlwx/k8s_admin/types"
)

func Get_spec_endpoints(namespace string, name string) *types.Endpoints {
	conten := common.API_URL("readSpecifiedEndpoints", namespace, name)
	fmt.Println("Get_spec_endpoints.conten", conten)
	httpcode, res := models.Get_json_Strem(conten)
	if httpcode != 200 {
		fmt.Println("httpcode:", httpcode)
	}
	//fmt.Println("res=", res)
	s_end := &types.Endpoints{}
	json.Unmarshal(res, &s_end)
	//fmt.Println(s_end)
	return s_end
}
